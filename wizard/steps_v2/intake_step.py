...
INTAKE_PREFIX = profile_prefix(ProfilePaths.INTAKE_RAW_INPUT)

_SUMMARY_FIELDS = (ProfilePaths.INTAKE_SOURCE_LANGUAGE, ProfilePaths.INTAKE_TARGET_LOCALE)
...
def render_intake_step(context: WizardContext) -> None:
    ...
    top, optional = collect_top_questions(
        profile=profile, required_paths=required_paths, followup_prefixes=(INTAKE_PREFIX,)
    )
    ...
    with st.form("v2_intake_form"):
        raw_input = st.text_area(
            str(ProfilePaths.INTAKE_RAW_INPUT),
            value=str(get_value(profile, str(ProfilePaths.INTAKE_RAW_INPUT)) or ""),
            height=160,
        )
        source_language = st.text_input(
            str(ProfilePaths.INTAKE_SOURCE_LANGUAGE),
            value=str(get_value(profile, str(ProfilePaths.INTAKE_SOURCE_LANGUAGE)) or ""),
        )
        target_locale = st.text_input(
            str(ProfilePaths.INTAKE_TARGET_LOCALE),
            value=str(get_value(profile, str(ProfilePaths.INTAKE_TARGET_LOCALE)) or ""),
        )
        submitted = st.form_submit_button(tr("Änderungen speichern", "Save changes"), type="primary")

    if submitted:
        updates: dict[str, Any] = {
            str(ProfilePaths.INTAKE_RAW_INPUT): raw_input.strip(),
            str(ProfilePaths.INTAKE_SOURCE_LANGUAGE): source_language.strip(),
            str(ProfilePaths.INTAKE_TARGET_LOCALE): target_locale.strip(),
        }
        commit_profile(profile, updates, context_update=context.update_profile)
        st.success(tr("Intake gespeichert.", "Intake saved."))

    tools_questions = collect_followup_questions(profile=profile, followup_prefixes=(INTAKE_PREFIX,))
    if tools_questions:
