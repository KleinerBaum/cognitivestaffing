...
def chip_multiselect(
    ...
    *,
    ...
    # ``state_key`` ensures widget identifiers remain stable independent of UI
    # translations.  When provided, it serves as the canonical root for all
    # session keys generated by this helper.
    ...
    if canonical_root:
        ...
        slug_parts.append(_slugify_label(label))
    if key_suffix:
        suffix = str(key_suffix).strip()
        if suffix:
            if state_key:
                slug_parts.append(suffix)
            else:
                slug_parts.append(_slugify_label(suffix))
