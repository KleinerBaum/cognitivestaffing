...
- ESCO HTTP GET handling now includes transient-failure retries with exponential backoff, sanitized technical logging, and a session-level fallback flag so the wizard can show a bilingual "local suggestions used" hint when API calls fail.
- ESCO cache keys are now standardized across core + wizard layers (`query`/`uri`, language, limit, cache API version), and cache-layer responsibilities are documented to limit duplicate raw caching.
...
- Added targeted schema diagnostics before Chat Completions calls: logs now include a `response_format meta-schema probe` (properties/required diff for `meta`) to isolate `field_metadata` mismatches quickly.
- Need-analysis extraction now sources its JSON schema from the shared response schema registry first (with fallback), reducing drift between extraction and response-format validation paths.
- On unrecoverable `response_format` schema errors, schema caches are cleared automatically and a restart hint is logged to avoid stale-worker schema mismatches.
...
### Changed
- Company web research now executes threaded page fetches through a Streamlit-free worker helper; cache/UI/session writes stay on the main thread, and expected bare-mode ScriptRunContext warnings are filtered to reduce log noise.
- Recoverable wizard rendering errors now use a session-scoped signature counter (`error_type + field_key + step`) with duplicate-log suppression and an automatic follow-up degradation mode after repeated failures, including bilingual user-facing warnings.
...
- Landing-first regression coverage now verifies step registry order, canonical section indices, initial navigation routing, and sidebar landing-overview suppression to keep pre-wizard behavior stable.
