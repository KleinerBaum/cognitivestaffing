...
- If invalid JSON persists, switch to **Precise/Genau** mode to use a higher reasoning model. Legacy `USE_CLASSIC_API` toggles have been removed; routing stays on the Responses API.
...
- The client replays the same request without streaming and then falls back to Chat completions when needed. Legacy `USE_CLASSIC_API` overrides have been removed.
