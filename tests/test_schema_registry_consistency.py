...
from core.schema_registry import adapt_payload, get_canonical_json_schema, get_canonical_model
from models.need_analysis_v2 import NeedAnalysisV2


def _collect_schema_leaf_fields(schema: Mapping[str, Any], prefix: str = "") -> set[str]:
    ...
    for key, value in properties.items():
        ...
        if isinstance(nested_props, Mapping):
            fields.update(_collect_schema_leaf_fields(value, path))
            continue
        fields.add(path)
    return fields


def test_registry_v1_schema_matches_canonical_v1_field_set() -> None:
    schema = get_canonical_json_schema(schema_version="v1", artifact="need_analysis")
    schema_fields = _collect_schema_leaf_fields(schema)

    expected_fields = set(ALL_FIELDS) - {"business_context.source_confidence"}
    assert schema_fields == expected_fields


def test_registry_v2_schema_matches_canonical_v2_top_level_fields() -> None:
...
def test_v1_to_v2_adapter_export_payload_matches_v2_schema() -> None:
    v1_payload = {
