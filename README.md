...
* After uploading a job ad, an “Extraction settings” expander lets recruiters switch parsing mode (Fast vs. Thorough), pick the base model, and toggle strict JSON enforcement to recover ads that struggle with schema validation.
...
* Nach dem Hochladen einer Anzeige ermöglicht ein Aufklapper „Extraktionseinstellungen“, den Parsing-Modus (Schnell vs. Gründlich) zu wählen, das Basismodell festzulegen und die strikte JSON-Erzwingung zu deaktivieren, falls Anzeigen wiederholt an der Schema-Validierung scheitern.
