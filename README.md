...
* EN: Simplified the wizard canvas further – relocating the metadata-only step modules into `wizard_pages/` hides Streamlit's default multipage header for good, and the Summary step now renders captured requirement lists as clean bullet points instead of chip rows so navigation cues live exclusively inside the sidebar.
* DE: Wizard-Canvas weiter entschlackt – durch das Verschieben der Metadaten-Schritte in `wizard_pages/` verschwindet die Streamlit-Multipage-Leiste dauerhaft, und der Summary-Schritt zeigt erfasste Anforderungen jetzt als schlichte Aufzählungen statt Chip-Reihen, sodass sämtliche Navigationshinweise ausschließlich über die Sidebar laufen.
