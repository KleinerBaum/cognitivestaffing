...
* EN: Unified the bilingual terminology across the Team & Context and Skills steps – the wizard canvas, sidebar, salary insight cards, and job-ad exports now show “Company contact email/phone” plus “Required/Optional skills” everywhere so HR reviewers read the same story from intake through summary.
* DE: Terminologie in den Schritten „Team & Kontext“ und „Skills & Requirements“ vereinheitlicht – Wizard, Sidebar, Salary-Insights und Job-Ad-Export zeigen nun überall „Kontakt-E-Mail/-Telefon (Unternehmen)“ sowie „Pflicht-“ bzw. „Optionale Skills“, damit HR dieselben Begriffe auf allen Oberflächen sieht.
...
* EN: Streaming completions now detect missing `response.completed` events, transparently retry the request, and fall back to the Chat Completions API when needed so the wizard no longer shows partial output or noisy tracebacks when the Responses stream ends abruptly.
...
DE: Gehaltsschätzungen orientieren sich jetzt an Jobtitel, Kernaufgaben, Pflicht- und optionale Anforderungen, Tools/Technologien/Zertifikaten, Sprachvorgaben, Branche sowie der angegebenen Stadt; die Standard-Navigation von Streamlit blendet den überflüssigen Überblick-Eintrag aus. Im Prozess-Schritt sorgt ein numerisches Feld „Interviewstufen (Anzahl)“ dafür, dass `process.interview_stages` dem Integer-Schema entspricht.
