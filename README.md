...
* Extraction settings now prime both UI and backend session keys for strict JSON enforcement, preventing session-state errors when toggling strict mode while keeping the checkbox and parser aligned across reruns.
...
* Inline follow-up questions now ship with info/warning accents and a tinted critical state so unanswered gaps stand out, and AI-prefilled badges expose an “AI suggestion” label on hover for clearer accessibility cues.
* Responsibilities and requirements now render as stacked, full-width panels with adaptive text areas so long bullet lists stay readable on narrow screens without nested scrollbars.
...
* The “Next” button stays enabled even when required fields are empty; a bilingual hint beneath it flags missing data so recruiters can jump ahead and fill gaps later without losing context.
...
* Im Schritt „Aufgaben“ erscheinen Verantwortlichkeiten und Anforderungen jetzt untereinander als breite Panels mit dynamischer Texthöhe, damit lange Listen auf schmalen Screens ohne verschachtelte Scrollleisten lesbar bleiben.
...
* Die Extraktionseinstellungen nutzen jetzt einen einzigen Sitzungsschlüssel für die strikte JSON-Erzwingung, sodass Checkbox und Backend bei Wiederholungen synchron bleiben.
...
* Inline-Follow-up-Fragen nutzen nun Info-/Warn-Akzente und einen getönten kritischen Zustand, damit offene Lücken sofort auffallen; KI-Badges zeigen beim Hover den Hinweis „AI suggestion“/„KI-Vorschlag“ für bessere Zugänglichkeit.
...
* Der „Weiter“-Button bleibt auch bei fehlenden Pflichtfeldern aktiviert; ein zweisprachiger Hinweis darunter markiert fehlende Angaben, sodass Recruiter:innen weitergehen und Lücken später schließen können.
...
* EN: Schema-validation failures (for example missing `company.name`) no longer break the session: the extractor preserves the original payload, flags the critical gaps for the wizard, and keeps the UI hints visible so recruiters can fill them manually.
* DE: Schema-Validierungsfehler (z. B. fehlendes `company.name`) unterbrechen die Sitzung nicht mehr: Der Extraktor behält die ursprüngliche Antwort, markiert die kritischen Lücken für den Wizard und lässt die UI-Hinweise sichtbar, damit Recruiter:innen sie manuell ergänzen können.
...
* DE: Die regelbasierte Extraktion durchsucht Anzeigen jetzt nach allgemeinen E-Mail-/Telefonangaben, Standortzeilen sowie Reise- oder Teamgrößenangaben, damit Kontakt-, Standort- und Arbeitslast-Felder automatisch vor dem LLM-Aufruf befüllt werden.
* EN: German section detection now treats headings like "Jobbeschreibung" or "Tätigkeitsbeschreibung" as responsibilities and stops requirements capture when encountering benefits blocks such as "Wir bieten"/"Das bieten wir", reducing mingled task and skill lists.
* DE: Die Abschnittserkennung in deutschen Anzeigen interpretiert Überschriften wie „Jobbeschreibung“ oder „Tätigkeitsbeschreibung“ als Aufgaben und beendet die Anforderungs-Erfassung bei Benefit-Blöcken (z. B. „Wir bieten“/„Das bieten wir“), sodass Aufgaben- und Skill-Listen nicht mehr vermischt werden.
* EN: Skill extraction now separates hard skills, soft skills, languages, certifications, and tools into the NeedAnalysis requirements fields, with a keyword-based fallback classifier that rebalances collapsed lists automatically so mixed skills, tools, and languages land in the right buckets.
* DE: Die Skill-Extraktion trennt jetzt Hard Skills, Soft Skills, Sprachen, Zertifizierungen und Tools in die NeedAnalysis-Anforderungsfelder und nutzt einen schlüsselwortbasierten Fallback-Klassifizierer, der zusammengefasste Listen automatisch neu verteilt, damit gemischte Skills, Tools und Sprachen im richtigen Bucket landen.
* EN: Uploaded job ads now map \"Must Have Skills\" and \"Nice To Have Skills\" sections into required vs. optional requirements, keep German \"Anforderungen\" bullets under requirements, and route tooling into requirements.tools_and_technologies instead of merging everything into one list.
* DE: Hochgeladene Anzeigen ordnen \"Must Have Skills\"- und \"Nice To Have Skills\"-Abschnitte jetzt den Pflicht- bzw. Optional-Requirements zu, halten \"Anforderungen\"-Stichpunkte in den Requirements und verschieben Tools in requirements.tools_and_technologies statt alles zusammenzuwerfen.
* EN: German phrasing like "wünschenswert"/"von Vorteil" now guides optional skill buckets, programming languages and tooling (Python, Jira, Excel) map to tools_and_technologies, and language proficiency lines stay in requirements.languages_* instead of hard skills.
* DE: Deutsche Formulierungen wie „wünschenswert“/„von Vorteil“ steuern jetzt die Optional-Buckets, Programmiersprachen und Tools (Python, Jira, Excel) landen in tools_and_technologies und Sprachkenntnisse bleiben in requirements.languages_* statt bei den Hard Skills.
- EN: Compensation selectors now localize currency and pay-period options while keeping canonical values, and the custom "Other" currency path shows bilingual copy so German and English users see consistent labels.
  DE: Die Vergütungsfelder lokalisieren jetzt Währungs- und Periodenoptionen bei unveränderten Kanonwerten, und der freie "Andere"-Weg nutzt zweisprachige Texte, damit deutsch- und englischsprachige Nutzer:innen konsistente Labels sehen.
* EN: OpenAI schema injection is now centralized so both Responses and Chat payloads reuse the same helper, and the logging stack records when fallbacks trigger or invalid fields get pruned, making production debugging far less noisy. Schema validation errors (for example `invalid_json_schema` or missing required fields) now short-circuit retries so the app backs off immediately instead of spamming the API.
* DE: Die OpenAI-Schemaeinbindung ist nun zentralisiert, sodass Responses- und Chat-Payloads denselben Helfer verwenden; außerdem protokollieren wir, wann Fallbacks starten oder Felder entfernt werden – dadurch lassen sich Produktionsfehler deutlich schneller nachvollziehen. Schema-Validierungsfehler (z. B. `invalid_json_schema` oder fehlende Pflichtfelder) brechen Wiederholungen jetzt sofort ab, sodass die App direkt zurückfällt statt die API zu fluten.
* EN: A new OpenAI request adapter now strips unsupported fields before dispatch, maps Responses-style payloads onto chat fallbacks, and logs schema validation failures (including `invalid_json_schema`) as non-retriable so malformed requests are not replayed.
* DE: Ein neuer OpenAI-Request-Adapter entfernt nicht unterstützte Felder vor dem Versand, überträgt Responses-Payloads korrekt auf Chat-Fallbacks und protokolliert Schemafehler (inklusive `invalid_json_schema`) als nicht wiederholbar, sodass fehlerhafte Anfragen nicht erneut gesendet werden.
* EN: Shared OpenAI retry/backoff handling now lives in a reusable decorator, and chat vs. Responses payload builders are split into dedicated dataclasses so tests can target each path without wading through intertwined conditionals.
* DE: Gemeinsames Retry-/Backoff-Handling für OpenAI steckt jetzt in einem wiederverwendbaren Decorator, und die Payload-Builder für Chat bzw. Responses sind in eigene Dataclasses aufgeteilt, sodass Tests jede Variante abdecken können, ohne sich durch verschachtelte If/Else-Blöcke zu kämpfen.
* EN: Debug/API toggles plus any raw JSON diagnostics (salary estimates, ESCO metadata, salary factors) now live exclusively inside the admin-only expander – recruiters never see them unless `ADMIN_DEBUG_PANEL=1` and the debug mode is enabled inside that panel.
* DE: Debug-/API-Schalter sowie alle Roh-JSON-Diagnosen (Gehaltsschätzungen, ESCO-Metadaten, Gehaltsfaktoren) erscheinen nur noch im Admin-Expander – Recruiter:innen sehen sie ausschließlich, wenn `ADMIN_DEBUG_PANEL=1` gesetzt ist und der Debugmodus innerhalb dieses Panels aktiviert wurde.
* EN: Inline follow-up cards now push their answers into the NeedAnalysis profile via the widget return values or explicit defaults instead of writing to `st.session_state["<field>"]` after widget creation, preventing Streamlit from raising "session_state.<key> cannot be modified" errors when the canonical field already has an instantiated input elsewhere in the wizard.
* DE: Inline-Follow-up-Karten schreiben ihre Antworten jetzt ausschließlich über die Widget-Rückgabewerte bzw. initiale `value=`-Defaults ins NeedAnalysis-Profil, statt nach dem Rendern direkt `st.session_state["<feld>"]` zu manipulieren – dadurch verschwinden die Streamlit-Fehler „session_state.<key> cannot be modified“, wenn das kanonische Feld bereits durch ein anderes Eingabefeld belegt ist.
* EN: Any new follow-up implementation must keep using widget return values and `_update_profile(..., session_value=...)` to sync the NeedAnalysis profile; directly mutating canonical `st.session_state` keys after widgets mount is considered a regression because it desynchronises the sidebar badges and summary.
* DE: Neue Follow-ups müssen ihre Werte weiterhin über Widget-Rückgabewerte und `_update_profile(..., session_value=...)` in das NeedAnalysis-Profil schreiben; direkte Änderungen an kanonischen `st.session_state`-Keys nach dem Widget-Mount gelten als Regression, weil dadurch Sidebar-Badges und Summary aus dem Takt geraten.
* EN: The wizard now renders a bilingual warning banner on every step whenever extraction JSON had to be auto-repaired, including a bullet list of the affected fields so recruiters can immediately fix them without digging through logs.
* DE: Der Wizard zeigt bei reparierten Extraktionsantworten auf allen Schritten ein zweisprachiges Warnbanner samt Aufzählung der betroffenen Felder an, damit Recruiter:innen die Probleme ohne Log-Recherche direkt beheben können.
* EN: Validation repairs that insert placeholder defaults or drop invalid fields now trigger the same bilingual banner per step after the profile loads, listing the impacted schema paths so recruiters can double-check those inputs.
* DE: Validierungsreparaturen, die Platzhalter ergänzen oder ungültige Felder verwerfen, aktivieren nach dem Laden des Profils ebenfalls das zweisprachige Banner pro Schritt und listen die betroffenen Schema-Pfade auf, damit Recruiter:innen diese Eingaben gezielt prüfen können.
* EN: Candidate profile summaries now ingest the target-role requirements, write summary_text as a fit assessment, and store bilingual fit_notes entries that list match percentage, overlapping skills, and the missing qualifications the vacancy still needs.
* DE: Kandidatenprofile übernehmen jetzt die Zielrollen-Anforderungen, formulieren summary_text als Fit-Bewertung und schreiben zweisprachige fit_notes-Einträge mit Match-Prozent, überlappenden Skills sowie den Qualifikationen, die für die Vakanz noch fehlen.
* EN: Candidate matching now uses a deterministic scoring model that heavily penalises missing must-have skills, mildly adjusts for nice-to-have skills, experience, and location, and populates the `gaps` array with explicit missing qualifications so recruiters know why someone ranked lower.
* DE: Das Kandidaten-Matching setzt jetzt auf ein deterministisches Scoring, das fehlende Must-have-Skills stark abstraft, Nice-to-have-Skills, Erfahrung und Standort nur moderat gewichtet und das Feld `gaps` mit konkreten fehlenden Qualifikationen füllt, damit Recruiter:innen sofort erkennen, warum jemand niedriger eingestuft wurde.
* EN: Canonicalized every payload transform with the schema alias dictionaries so legacy keys such as `role.department` or `company.headquarters` automatically flow into the canonical fields before validation, export, or wizard syncing.
* DE: Sämtliche Payload-Transformationen wenden jetzt die Schema-Alias-Tabellen an, damit Legacy-Schlüssel wie `role.department` oder `company.headquarters` vor der Validierung, im Export und beim Wizard-Sync automatisch in die kanonischen Felder überführt werden.
* EN: Expanded the RecruitingWizard alias map (e.g., `role.department`, `role.team`, `company.hq`, brand colors, and logo URLs) and trimmed unknown payload fields before validation so legacy JSON imports keep loading without schema errors.
* DE: Die RecruitingWizard-Alias-Tabelle wurde erweitert (z. B. `role.department`, `role.team`, `company.hq`, Markenfarben und Logo-URLs) und unbekannte Payload-Felder werden vor der Validierung entfernt, damit Legacy-JSON-Importe weiterhin ohne Schemafehler geladen werden können.
* EN: Summary exports and the Boolean search helper now call `coerce_and_fill()` before building NeedAnalysisProfile instances so uploaded JSON or manual form edits that still use `role.*` and `company.headquarters` aliases keep rendering without validation errors.
* DE: Summary-Exporte und der Boolean-Suchhelfer rufen vor dem Aufbau des NeedAnalysisProfile jetzt `coerce_and_fill()` auf, damit hochgeladene JSONs oder manuelle Formulareingaben mit `role.*`- bzw. `company.headquarters`-Aliasfeldern weiterhin ohne Validierungsfehler angezeigt werden.
* EN: Streaming completions now detect missing `response.completed` events or empty final payloads, transparently replay the request in non-stream mode, and only then fall back to the Chat Completions API when needed so the wizard no longer shows partial output or noisy tracebacks when the Responses stream ends abruptly.
* DE: Streaming-Antworten erkennen fehlende `response.completed`-Events oder leere Final-Payloads, spielen die Anfrage transparent ohne Streaming erneut ab und greifen nur bei Bedarf auf die Chat-Completions-API zurück, sodass der Wizard bei abrupt endenden Responses-Streams keine Teilantworten oder lauten Tracebacks mehr anzeigt.
...
* DE: Die strukturierten Responses-Pipelines erzwingen jetzt `additionalProperties: false` in sämtlichen JSON-Schemas und verlangen beim Job-Ad-Schema den Metadatenblock (Ton und Zielgruppe), sodass weder unerwartete Felder noch fehlender Kontext bis zum Wizard durchdringen.
...
EN: The Company section now recomputes its required fields by rerunning the contact email and primary city validators on each rerun, so stale widget text that got wiped from the profile still triggers the bilingual “field required” notice and disables “Next” until fresh data is saved.
DE: Der Unternehmensabschnitt berechnet seine Pflichtfelder nun bei jedem Rerun neu, indem er die Validatoren für Kontakt-E-Mail und Primärstadt erneut ausführt – selbst wenn das Profil bereits geleert wurde, sorgt zurückgebliebener Widget-Text dafür, dass der zweisprachige Hinweis „Dieses Feld ist erforderlich“ erscheint und „Weiter“ gesperrt bleibt, bis neue Daten gespeichert wurden.
...
EN: Added a bilingual “🔄 Reset wizard” button to the sidebar settings so recruiters can instantly clear the current profile and reload the default wizard state in one click (without changing theme, language, or LLM preferences). Resetting now also purges every inline follow-up card and their `fu_*` focus markers so the sidebar starts clean.
DE: Einen zweisprachigen Button „🔄 Zurücksetzen / Reset wizard“ in den Seiteneinstellungen hinzugefügt, mit dem Recruiter:innen das aktuelle Profil mit einem Klick entfernen und den Wizard mit Standardwerten neu laden können (ohne Dark-Mode-, Sprach- oder LLM-Einstellungen zu verändern). Beim Zurücksetzen werden außerdem alle Inline-Follow-up-Karten samt `fu_*`-Fokusmarkern entfernt, damit die Sidebar garantiert leer startet.
...
EN: Schema repair now runs immediately after parsing to coerce, prune, and fill missing critical sections (for example `company.name`) with safe defaults, and the warning banner lists the auto-filled paths before any wizard widgets render.
DE: Die Schema-Reparatur greift jetzt direkt nach dem Parsen, bereinigt ungültige Felder, füllt fehlende kritische Abschnitte (z. B. `company.name`) mit sicheren Standardwerten und nennt die automatisch ergänzten Pfade in der Warnleiste, bevor irgendein Wizard-Widget angezeigt wird.
EN: Responsibilities, company-culture notes, and hiring-process timelines are enforced as critical sections—structured extraction fails fast when any of them are blank, prompting the assistant to re-ask before persisting an incomplete profile, and the structured Job Ad generator now rejects outputs with empty overview, responsibility, requirement, how-to-apply, or equal-opportunity sections (or missing target audiences) to keep exports HR-ready.
DE: Verantwortlichkeiten, Kulturhinweise und Prozesszeitpläne gelten jetzt als kritische Abschnitte – die strukturierte Extraktion bricht ab, sobald einer dieser Bereiche leer bleibt und zwingt zu erneuten Rückfragen, und der strukturierte Stellenanzeigen-Generator weist Ausgaben mit leerem Überblick, Aufgaben-, Anforderungs-, Bewerbungs- oder Equal-Opportunity-Abschnitt (bzw. fehlender Zielgruppe) zurück, damit alle Exporte den HR-Standards entsprechen.
...
EN: `schema/need_analysis.schema.json` is generated straight from NeedAnalysisProfile and the vacancy extraction schema reuses the same property definitions, guaranteeing that every field captured during extraction validates cleanly and reaches the UI plus exports without manual drift. The system prompts reuse the same field list so newly added schema paths appear automatically in extraction instructions.

DE: `schema/need_analysis.schema.json` wird direkt aus NeedAnalysisProfile erzeugt und das Vacancy-Extraktionsschema übernimmt dieselben Eigenschaftsdefinitionen, sodass alle Felder aus der Extraktion ohne manuelle Abweichungen die Validierung, die UI und die Exporte erreichen. Die System-Prompts nutzen dieselbe Feldliste, sodass neu hinzugefügte Schema-Pfade automatisch in den Extraktionsanweisungen erscheinen.

EN: Run `pytest tests/test_generate_interview_guide.py::test_interview_guide_model_dump_matches_schema tests/test_normalization.py::test_normalize_profile_pipeline_applies_aliases tests/test_responses_schema.py::test_need_analysis_schema_file_in_sync tests/test_schema_alignment.py::test_prompt_schema_and_model_fields_align` after touching the NeedAnalysis model, alias dictionaries, or interview-guide prompts. If the schema file drifts, regenerate it via `python scripts/propagate_schema.py --apply`.

DE: Führe `pytest tests/test_generate_interview_guide.py::test_interview_guide_model_dump_matches_schema tests/test_normalization.py::test_normalize_profile_pipeline_applies_aliases tests/test_responses_schema.py::test_need_analysis_schema_file_in_sync tests/test_schema_alignment.py::test_prompt_schema_and_model_fields_align` aus, sobald NeedAnalysis-Modelle, Alias-Tabellen oder Interview-Guide-Prompts geändert werden. Bei Schemaabweichungen `python scripts/propagate_schema.py --apply` starten, um `schema/need_analysis.schema.json` zu aktualisieren.
