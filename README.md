...
* After uploading a job ad, an “Extraction settings” expander lets recruiters switch parsing mode (Fast vs. Thorough), pick the base model, toggle strict JSON enforcement to recover ads that struggle with schema validation, and trigger a re-run with the updated settings.
* Extraction settings now share a single session key for strict JSON enforcement, keeping the checkbox and backend aligned across reruns.
...
* Nach dem Hochladen einer Anzeige ermöglicht ein Aufklapper „Extraktionseinstellungen“, den Parsing-Modus (Schnell vs. Gründlich) zu wählen, das Basismodell festzulegen, die strikte JSON-Erzwingung zu deaktivieren, falls Anzeigen wiederholt an der Schema-Validierung scheitern, und mit den neuen Einstellungen erneut zu starten.
