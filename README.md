...
* EN: OpenAI schema injection is now centralized so both Responses and Chat payloads reuse the same helper, and the logging stack records when fallbacks trigger or invalid fields get pruned, making production debugging far less noisy.
* DE: Die OpenAI-Schemaeinbindung ist nun zentralisiert, sodass Responses- und Chat-Payloads denselben Helfer verwenden; außerdem protokollieren wir, wann Fallbacks starten oder Felder entfernt werden – dadurch lassen sich Produktionsfehler deutlich schneller nachvollziehen.
