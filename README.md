...
- **ESCO integration:** When enabled, the ESCO enricher normalizes job titles, proposes essential skills, and flags missing competencies directly in the UI. **EN:** All occupation searches and essential-skill lookups now flow through a shared `st.cache_data` layer (24 h TTL) so repeated titles reuse the same ESCO response and the logs clearly show cache hits vs refreshes. **DE:** Sämtliche Berufs- und Skill-Abfragen laufen nun über einen gemeinsamen `st.cache_data`-Cache (24 h TTL), sodass identische Jobtitel dieselbe ESCO-Antwort wiederverwenden und die Logs Cache-Treffer sowie Aktualisierungen transparent machen.
