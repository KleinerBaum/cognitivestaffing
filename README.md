...
- **Company insights assistant**: On the Company step, a bilingual assistant pulls lightweight public hints (sample catalogue + Wikipedia/auto-complete) to prefill industry, size, HQ, website, or a short blurb – and if nothing is found, it asks targeted questions to collect the missing company profile. (DE: Im Schritt „Unternehmen“ liefert ein zweisprachiger Assistent öffentliche Hinweise aus einem Offline-Katalog plus Wikipedia/Autocomplete, um Branche, Größe, Hauptsitz, Website oder Kurzbeschreibung vorzufüllen – andernfalls stellt er gezielte Fragen, um die fehlenden Felder einzusammeln.)
- **Responsibility brainstormer**: The Role & Tasks step now hosts a ChatKit helper that proposes role-specific responsibilities, lets you add or dismiss each bullet, and accepts custom focus hints (DE: Der Schritt „Rolle & Aufgaben“ enthält einen ChatKit-Assistenten, der rollenbezogene Verantwortlichkeiten vorschlägt, jede Zeile per Klick übernehmen/ablehnen lässt und eigene Fokus-Hinweise berücksichtigt.).
- **Skill-set expander assistant**: On the Skills & Requirements step, an AI helper proposes related skills or certifications and lets you classify them as required or nice-to-have with one click (DE: KI schlägt ergänzende Skills/Zertifikate vor und du ordnest sie per Klick als Pflicht oder Optional zu.).
- **Compensation Range Assistant**: The compensation step now hosts an AI salary advisor that proposes realistic ranges with market hints, converts monthly budgets to annual figures, and can apply the min/max values (DE: Im Vergütungs-Schritt schlägt ein KI-Gehaltsassistent marktnahe Spannen vor, rechnet Monatsbudgets auf Jahreswerte hoch und trägt Min/Max auf Wunsch direkt ein.).
...
- **AI transparency in summary**: AI-suggested entries are italicised in the Summary view with a bilingual note so teams can verify additions before exporting. (DE: KI-Vorschläge erscheinen in der Zusammenfassung kursiv mit Hinweis, damit Teams Ergänzungen vor dem Export prüfen können.)
...
- **Recoverable wizard errors**: When a step fails (e.g., AI bad request or missing help text), the wizard now surfaces clear bilingual guidance so users can retry or fill fields manually while navigation stays responsive. (DE: Schlägt ein Schritt fehl – etwa wegen einer KI-Bad-Request oder fehlender Hilfetexte – zeigt der Wizard jetzt verständliche Hinweise an, damit Nutzer:innen erneut versuchen oder die Felder manuell ergänzen können, ohne dass die Navigation blockiert.)
