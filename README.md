...
The onboarding step starts with a single hero block (logo + headline/subheadline) followed immediately by the URL/upload call-to-action so the first screen is focused and uncluttered. Secondary messaging stays in helper text or expanders to keep the value prop → action flow clear.
